    ...
    .then((calendarEvent) => {
      if (calendarEvent) {
        res
          .status(200)
          .json({
            message: "calendarEvent fetched sucessfully!",
            calendarEvent: calendarEvent,
          });
      } else {
        res.status(404).json({ message: "calendarEvent not found!" });
      }
    })
    .catch((error) => {
      res.status(500).json({ message: "Fetching calendarEvent failed!" });
    });
};
...
exports.getNextEvents = (req, res, next) => {
  CalendarEvent.find({ roomId: req.params.roomId })
    .then((calendarEvents) => {
      ...
      if (true) {
        res.status(200).json({
          message: "Next 3 Events fetched successfullyyyy",
          today: today,
    ...
    .catch((error) => {
      res.status(500).json({ message: "Could not fetch next 3 events!" });
    });
};

exports.deleteCalendarEvent = (req, res, next) => {
  CalendarEvent.deleteOne({ _id: req.params.id, creator: req.userData.userId })
    .then((result) => {
...
exports.updateOne = (req, res, next) => {
  ...
  CalendarEvent.updateOne(
    ...
    .catch((error) => {
      res.status(500).json({ message: "Editing event failed!" });
    });
};
